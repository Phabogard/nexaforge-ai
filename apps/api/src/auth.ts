...
export function signToken(user: AuthUser, ttlSeconds = 60 * 60 * 8): string {
  ...
  const payload = b64(JSON.stringify({ sub: user.id, email: user.email, role: user.role, iat: Math.floor(Date.now() / 1000), exp: Math.floor(Date.now() / 1000) + ttlSeconds }));
  const input = `${header}.${payload}`;
  const signature = b64(createHmac('sha256', key).update(input).digest());
