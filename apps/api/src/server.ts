...
app.get('/health', async () => ({
  ...
  persistence: repository ? 'postgres' : 'memory',
}));
...
app.post('/api/v1/tasks', async (request, reply) => {
  ...
  if (repository && !parsed.data.workspaceId) {
    return reply.code(400).send({ error: 'WORKSPACE_REQUIRED', message: 'workspaceId is required when PostgreSQL persistence is enabled.' });
  }
    ...
    const task = repository
      ...
      : (() => {
          ...
          const created: TaskRecord = {
            ...
            status: 'queued',
            workspaceId: parsed.data.workspaceId ?? 'local-dev',
